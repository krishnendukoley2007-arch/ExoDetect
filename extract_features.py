...
PATCH 5 (2026-07-05): wrapped the run loop in main() + __main__ guard.
Before this, `import extract_features` from anywhere (a REPL, another
script, a one-off debug command) re-ran the ENTIRE 2,499-star extraction
from scratch. Now the module can be safely imported to reuse download_lc(),
run_bls_wide(), etc. without side effects. The full run only happens when
you execute `python extract_features.py` directly.
"""
